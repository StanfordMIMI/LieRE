...
def _select_fold_ucf101(
    data_path: str, video_list: List[str], annotations_dir: str, fold: int, mode: bool
) -> List[int]:
    mode = "train" if mode else "test"
    name = f"{mode}list{fold:02d}.txt"
    f = os.path.join(annotations_dir, name)
    selected_files = set()
    with open(f) as fid:
        data = fid.readlines()
        data = [x.strip().split(" ")[0] for x in data]
        data = [os.path.join(data_path, *x.split("/")) for x in data]
        selected_files.update(data)
    indices = [i for i in range(len(video_list)) if video_list[i] in selected_files]
    return indices


def _select_fold_hmdb51(
    data_path: str, video_list: List[str], annotations_dir: str, fold: int, mode: bool
) -> List[int]:
    target_tag = 1 if mode else 2
...
class VideoClsDataset(Dataset):
    ...
    def __init__(
        ...
        # self.dataset_samples = cleaned["clip_path"].tolist()
        # self.label_array = cleaned["int_labels"].tolist()
        extensions = ("avi",)
        self.classes, class_to_idx = find_classes(self.data_path)
        ...
        # all the samples in dataset
        video_list = [x[0] for x in self.samples]
        label_list = [x[1] for x in self.samples]

        # splitting, get indices of the train or validation set
        assert hasattr(args, "data_set"), "Missing dataset."
        if args.data_set == "ucf101":
            self.indices = _select_fold_ucf101(
                data_path=self.data_path,
                video_list=video_list,
                annotations_dir=self.anno_path,
                fold=1,
                mode=True if mode == "train" else False,
            )
        elif args.data_set == "hmdb51":
            self.indices = _select_fold_hmdb51(
                data_path=self.data_path,
                video_list=video_list,
                annotations_dir=self.anno_path,
                fold=1,
            ...
            print("No dataset selected, must be 'ucf101' or 'hmdb51'.")
            raise NotImplementedError

        if mode == "train":
            self.dataset_samples = [video_list[i] for i in self.indices]
            self.label_array = [label_list[i] for i in self.indices]
            pass
            ...
            for ck in range(self.test_num_segment):
                for cp in range(self.test_num_crop):
                    ...
                    for idx in self.indices:
                        self.test_label_array.append(label_list[idx])
                        self.test_dataset.append(video_list[idx])
